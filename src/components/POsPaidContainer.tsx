...
const POsPaidContainer: React.FC<POsPaidProps> = ({
  ...
  const loadAllFilterValues = async () => {
    try {
      ...
      // Load all filter values in parallel using apiService with correct field IDs
      const [vendorValues, warehouseValues, statusValues] = await Promise.all([
        apiService.getFilterValues('pomVendorNum'), // Vendor field ID
        apiService.getFilterValues('Whse'), // Warehouse field ID
        apiService.getFilterValues('Staic'), // Status field ID
      ]);
  ...
  const handleSearch = async (values: POsPaidSearchFormData) => {
    try {
      ...
      // Use apiService to search POs Paid data
      console.log('🔄 POsPaidContainer - Calling apiService.searchPOsPaid...');
      const results = await apiService.searchPOsPaid(values);

      console.log('✅ POsPaidContainer - Search results from API:', results);
      ...
      message.success(`Found ${results.length} POs paid records`);
    } catch (error) {
