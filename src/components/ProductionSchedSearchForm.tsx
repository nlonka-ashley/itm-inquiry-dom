...
const ProductionSchedSearchForm: React.FC<ProductionSchedSearchFormProps> = ({
  ...
  const handleFinish = (values: any) => {
    ...
    console.log("🔍 ProductionSchedSearchForm - Calling onSearch with:", searchData);
    onSearch(searchData);
    } catch (error) {
      console.error("❌ ProductionSchedSearchForm - Error in handleFinish:", error);
    }
  };
  ...
  const handleReset = () => {
    try {
      ...
      console.error("❌ ProductionSchedSearchForm - Error in handleReset:", error);
    }
  };
          ...
          >
            ...
            >
              {(filterValues.vendorValue || [])
                .filter((value) => value && value.filterId !== undefined && value.filterId !== null)
                .filter((value) => value.filterDesc && value.filterDesc.trim() !== "")
                .map((value, index) => (
                <Select.Option key={`vendor-${value.filterId}-${index}`} value={value.filterId}>
          ...
          >
            ...
            >
              {(filterValues.warehouseValue || [])
                ...
                .filter((value) => value.filterDesc && value.filterDesc.trim() !== "")
                .map((value, index) => (
                <Select.Option key={`warehouse-${value.filterId}-${index}`} value={value.filterId}>
                  {`${value.filterDesc} (${value.filterId})`}
                </Select.Option>
              ))}
            </Select>
              ...
              <>
                  ...
                  <Col xs={24} sm={12} md={8} lg={6}>
                    ...
                    >
                      <Select
                        ...
                        size="small"
                        optionFilterProp="children"
                        onChange={() => onFieldChange("ProductionResource", "productionResourceValue")}
                      >
                        {(filterValues.productionResourceValue || [])
                          .filter((value) => value && value.filterId !== undefined && value.filterId !== null)
                          .filter((value) => value.filterDesc && value.filterDesc.trim() !== "")
                          .map((value, index) => (
                          <Select.Option key={`production-resource-${value.filterId}-${index}`} value={value.filterId}>
                            {`${value.filterDesc} (${value.filterId})`}
                          </Select.Option>
                        ))}
                      </Select>
                    ...
                    >
                      ...
                      >
                        {(filterValues.drpValue || [])
                          ...
                          .filter((value) => value.filterDesc && value.filterDesc.trim() !== "")
                          .map((value, index) => (
                          <Select.Option key={`drp-${value.filterId}-${index}`} value={value.filterId}>
                            {`${value.filterDesc} (${value.filterId})`}
                          </Select.Option>
                        ))}
                      </Select>
                    ...
                    >
                      ...
                      >
                        {(filterValues.fcValue || [])
                          ...
                          .filter((value) => value.filterDesc && value.filterDesc.trim() !== "")
                          .map((value, index) => (
                          <Select.Option key={`fc-${value.filterId}-${index}`} value={value.filterId}>
                            {`${value.filterDesc} (${value.filterId})`}
                          </Select.Option>
                        ))}
                      </Select>
                    ...
                    >
                      ...
                      >
                        {(filterValues.officeValue || [])
                          ...
                          .filter((value) => value.filterDesc && value.filterDesc.trim() !== "")
                          .map((value, index) => (
                          <Select.Option key={`office-${value.filterId}-${index}`} value={value.filterId}>
                            {`${value.filterDesc} (${value.filterId})`}
                          </Select.Option>
                        ))}
                      </Select>
                  ...
                  <Col xs={24} sm={12} md={8} lg={6}>
                    ...
                    >
                      <Select
                        ...
                        size="small"
                        optionFilterProp="children"
                        onChange={() => onFieldChange("ItemClass", "itemClassValue")}
                      >
                        {(filterValues.itemClassValue || [])
                          .filter((value) => value && value.filterId !== undefined && value.filterId !== null)
                          .filter((value) => value.filterId && value.filterId.trim() !== "")
                          .map((value, index) => (
                          <Select.Option key={`item-class-${value.filterId}-${index}`} value={value.filterId}>
                            {value.filterId}
                          </Select.Option>
                        ))}
                      </Select>
                ...
                <Row gutter={[24, 16]} justify="center">
                  <Col xs={24} sm={20} md={16} lg={12}>
                    <div>
                      <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>Filter Options</label>
                      <Row gutter={[4, 4]}>
                        <Col xs={24} sm={12}>
                          <Form.Item
                            name="rpFilter"
                  ...
                  <Col xs={24} sm={20} md={16} lg={12}>
                    <div>
                      <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>Order Types</label>
                      <Row gutter={[16, 8]}>
                        <Col xs={24} sm={8}>
                          <Form.Item
                            name="plannedOrders"
