...
const API_CONFIG = {
  ...
  PRODUCTION_SCHEDULE_API_URL: (env: string) =>
    `/api/ProductionSchedule/${env}/search`,
  ...
  try {
    ...
    switch (method) {
      ...
      case 'put':
        ...
        break;
      case 'patch':
        response = await axiosInstance.patch<T>(
          url,
          configWithParams.data,
          configWithParams,
        );
...
export class ApiService {
  ...
  private static isCacheValid(key: string): boolean {
    const expiry = ApiService.cacheExpiry.get(key);
    return expiry ? Date.now() < expiry : false;
  }

  private static setCache<T>(key: string, data: T): void {
    ApiService.cache.set(key, data);
    ApiService.cacheExpiry.set(key, Date.now() + ApiService.CACHE_DURATION);
  }

  private static getCache<T>(key: string): T | null {
    if (ApiService.isCacheValid(key)) {
      return ApiService.cache.get(key) as T;
    }
    return null;
  }
  ...
  static async getFilterValues(fieldId: string): Promise<FilterValue[]> {
    ...
    const cached = ApiService.getCache<FilterValue[]>(cacheKey);
    if (cached) {
    ...
    try {
      ...
      ApiService.setCache(cacheKey, filterValues);
      return filterValues;
    } catch (error: any) {
...
export async function fetchProductionVendors(): Promise<FilterValue[]> {
  try {
    ...
    const data = await fetchAPI<any>(API_CONFIG.PRODUCTION_VENDORS_API_URL, {
      params,
    });
    ...
    if (data?.data && Array.isArray(data.data)) {
      ...
      console.warn(
        '🔍 PRODUCTION VENDORS - Unexpected response structure:',
        data,
      );
      return [];
    }
    ...
    console.log(
      '✅ PRODUCTION VENDORS - Extracted count:',
...
export async function fetchProductionResources(
  vendorId?: string,
): Promise<ProductionResourceInfo[]> {
  // Return static 'ALL Production Resource' option since API is not available
  console.log(
    "🔍 PRODUCTION RESOURCES - Returning static 'ALL Production Resource' option",
  );
  return [
    {
      resourceId: 'ALL',
...
export async function fetchItemClasses(): Promise<ItemClassInfo[]> {
  try {
    ...
    const data = await fetchAPI<any>(API_CONFIG.ITEM_CLASSES_API_URL, {
      params,
    });
    console.log(
      '🔍 ITEM CLASSES - Raw API response:',
      JSON.stringify(data, null, 2),
    );
    ...
    if (itemArray.length > 0) {
      ...
      console.log(
        '🔍 ITEM CLASSES - Available fields:',
        Object.keys(itemArray[0]),
      );
    }
  ...
  try {
    ...
    if (data && typeof data === 'object') {
      ...
      if (data.data && Array.isArray(data.data) && data.data.length > 0) {
        console.log(
          '🔍 DRP CODES - First data item:',
          JSON.stringify(data.data[0], null, 2),
        );
        console.log(
          '🔍 DRP CODES - First data item keys:',
    ...
    console.log(
      '� DRP CODES - Using mock data temporarily to fix display issue',
    );
    ...
    if (data?.data && Array.isArray(data.data)) {
      ...
      console.log(
        '🔄 DRP CODES - Using mock data for unexpected response structure',
      );
      return [
    ...
    if (drpArray.length > 0) {
      console.log(
        '🔍 DRP CODES - First item structure:',
        JSON.stringify(drpArray[0], null, 2),
      );
...
export async function fetchFCCodes(): Promise<FCInfo[]> {
  try {
    ...
    console.log(
      '🔍 FC CODES - Raw API response:',
      JSON.stringify(data, null, 2),
    );

    // Temporarily return mock data until we fix the field mapping
    console.log(
      '� FC CODES - Using mock data temporarily to fix display issue',
    );
...
class ProductionSchedApiService {
  ...
  async getFilterValues(filterType: string): Promise<FilterValue[]> {
    try {
      console.log(
        `🔄 Getting Production Schedule filter values for: ${filterType}`,
      );
      ...
      switch (filterType) {
        ...
        default:
          console.warn(
            `Unknown Production Schedule filter type: ${filterType}`,
          );
          return [];
      }

      console.log(
        `✅ Retrieved ${data.length} Production Schedule values for ${filterType}`,
      );
      return data;
    } catch (error) {
      console.error(
        `❌ Error getting Production Schedule filter values for ${filterType}:`,
        error,
      );
      return [];
    }
  ...
  ): Promise<ProductionSchedData[]> {
    try {
      console.log(
        '🔄 Searching Production Schedule with real API:',
        searchData,
      );
      ...
      const apiRequest: ProductionSchedApiRequest = {
        ...
        warehouse:
          this.getFilterValue(searchData.filterRows, 'Warehouse') || '',
        ...
        excludeContainerDirect: searchData.reportOptions.containerDirectFilter
          ? 0
          : 1,
        productionResource:
          this.getFilterValue(searchData.filterRows, 'ProductionResource') ||
          '',
        itemClass:
          this.getFilterValue(searchData.filterRows, 'ItemClass') || '',
      };
      ...
      console.log(
        `✅ Production Schedule search completed: Found ${results.length} items`,
      );
  ...
  private getFilterValue(filterRows: any[], fieldType: string): string {
    const filter = filterRows.find(
      row => row.fieldType === fieldType && row.isActive,
    );
...
export async function debugProductionScheduleAPIs(): Promise<void> {
  ...
  try {
    ...
    const drpResponse = await fetchAPI<any>(API_CONFIG.DRP_API_URL, {
      params: { environment: 'afi' },
    });
    console.log('📊 DRP Response Structure:', {
      type: typeof drpResponse,
      isArray: Array.isArray(drpResponse),
      keys:
        drpResponse && typeof drpResponse === 'object'
          ? Object.keys(drpResponse)
          : 'N/A',
      hasData: !!drpResponse?.data,
      dataLength:
        drpResponse?.data && Array.isArray(drpResponse.data)
          ? drpResponse.data.length
          : 'N/A',
    });

    if (
      drpResponse?.data &&
      Array.isArray(drpResponse.data) &&
      drpResponse.data.length > 0
    ) {
      console.log(
        '📋 DRP First Item:',
        JSON.stringify(drpResponse.data[0], null, 2),
      );
    ...
    const fcResponse = await fetchAPI<any>(API_CONFIG.FC_API_URL, {
      params: { environment: 'afi' },
    });
    console.log('📊 FC Response Structure:', {
      ...
      keys:
        fcResponse && typeof fcResponse === 'object'
          ? Object.keys(fcResponse)
          : 'N/A',
      hasData: !!fcResponse?.data,
      dataLength:
        fcResponse?.data && Array.isArray(fcResponse.data)
          ? fcResponse.data.length
          : 'N/A',
    });

    if (
      fcResponse?.data &&
      Array.isArray(fcResponse.data) &&
      fcResponse.data.length > 0
    ) {
      console.log(
        '📋 FC First Item:',
        JSON.stringify(fcResponse.data[0], null, 2),
      );
    ...
    const itemResponse = await fetchAPI<any>(API_CONFIG.ITEM_CLASSES_API_URL, {
      params: { environment: 'afi' },
    });
    console.log('� Item Classes Response Structure:', {
      ...
      keys:
        itemResponse && typeof itemResponse === 'object'
          ? Object.keys(itemResponse)
          : 'N/A',
      hasData: !!itemResponse?.data,
      dataLength:
        itemResponse?.data && Array.isArray(itemResponse.data)
    ...
    if (
      ...
      console.log(
        ...
        JSON.stringify(itemResponse.data[0], null, 2),
      );
      console.log(
        '📋 Item Classes First Item Keys:',
        Object.keys(itemResponse.data[0]),
      );
    }
  } catch (error) {
