...
const mockPOItems: POItemData[] = (() => {
  ...
  for (let i = 1; i <= 500; i++) {
    ...
    items.push({
      ...
      itemDesc: furnitureItems[Math.floor(Math.random() * furnitureItems.length)],
      ...
      shipDate: new Date(Date.now() + Math.random() * 15 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
      xpectedDelivery: new Date(Date.now() + Math.random() * 45 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
