...
import type { POItemData, ProductionSchedData, ProductionSchedSearchFormData, POsPaidData } from "../types";
...
export class ExcelExportService {
   ...
   * @param filename Optional filename (defaults to generated name with timestamp)
   */
  static exportProductionScheduleToExcel(data: ProductionSchedData[], filename?: string): void {
    try {
      // Prepare data for Excel export
      const excelData = data.map((item, index) => ({
        "Row #": index + 1,
      ...
      console.log(`✅ Production Schedule Excel export completed: ${finalFilename}`);
      console.log(`📊 Exported ${data.length} records`);
    } catch (error) {
  ...
  static exportProductionScheduleWithCriteria(
    ...
    try {
      ...
      const criteriaData = [
        { Field: "Export Date", Value: dayjs().format("YYYY-MM-DD HH:mm:ss") },
        { Field: "Total Records", Value: data.length },
        { Field: "Export Type", Value: "Production Schedule with Search Criteria" },
        { Field: "", Value: "" }, // Empty row
        { Field: "SEARCH CRITERIA", Value: "" },
        { Field: "Active Filter Rows", Value: searchCriteria.filterRows.length },
        ...searchCriteria.filterRows.map((row, index) => ({
          Field: `Filter ${index + 1}`,
          Value: `${row.logicalOperator || ""} ${row.fieldType} ${row.comparisonOperator} "${row.filterValue}"`.trim(),
        })),
        { Field: "", Value: "" }, // Empty row
        { Field: "ORDER TYPE FILTERS", Value: "" },
        { Field: "Planned Orders", Value: searchCriteria.orderTypeFilters.plannedOrders ? "Yes" : "No" },
        { Field: "Firmed Orders", Value: searchCriteria.orderTypeFilters.firmedOrders ? "Yes" : "No" },
        { Field: "Shipped Orders", Value: searchCriteria.orderTypeFilters.shippedOrders ? "Yes" : "No" },
        { Field: "", Value: "" }, // Empty row
        ...
        { Field: "Container Direct Filter", Value: searchCriteria.reportOptions.containerDirectFilter ? "Yes" : "No" },
      ];
      ...
      this.exportProductionScheduleToExcel(data, filename);

      console.log(`✅ Production Schedule Excel export with criteria completed`);
      console.log(`📊 Exported ${data.length} records with search criteria`);
    } catch (error) {
      console.error("❌ Production Schedule Excel export with criteria failed:", error);
      throw new Error("Failed to export Production Schedule Excel file with criteria");
    }
  }
  ...
  static exportPOsPaidData(data: POsPaidData[], filename?: string): void {
    try {
      ...
      const excelData = data.map((item, index) => ({
        ...
        "Date Paid": item.paymentTerms === "Free Of Charge FOC" ? "Ineligible" :
                     item.pomDatePaid ? dayjs(item.pomDatePaid).format("MM/DD/YYYY") : "",
        ...
        "ETD Date": item.etdDate ? dayjs(item.etdDate).format("MM/DD/YYYY") : "",
        "ETA Date": item.etaDate ? dayjs(item.etaDate).format("MM/DD/YYYY") : "",
        "On Board Date": item.onboard ? dayjs(item.onboard).format("MM/DD/YYYY") : "",
        "Payment Terms": item.paymentTerms,
        "Pmt Approved Date": item.pmtApproveDate ? dayjs(item.pmtApproveDate).format("MM/DD/YYYY") : "",
      }));
      ...
      console.log("✅ ExcelExportService - POs Paid data exported successfully");
    } catch (error) {
      console.error("❌ ExcelExportService - Error exporting POs Paid data:", error);
